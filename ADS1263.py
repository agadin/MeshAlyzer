...
class ADS1263:
    ...
    def ADS1263_reset(self):
        config.digital_write(self.rst_pin, config.HIGH)
        config.delay_ms(200)
        config.digital_write(self.rst_pin, config.LOW)
        config.delay_ms(200)
        config.digital_write(self.rst_pin, config.HIGH)
        config.delay_ms(200)

    def ADS1263_WriteCmd(self, reg):
        config.digital_write(self.cs_pin, config.LOW)
        config.spi_writebyte([reg])
        config.digital_write(self.cs_pin, config.HIGH)
    ...
    def ADS1263_Read_ADC2_Data(self):
        ...
        while True:
            ...
            if config.spi_readbytes(1)[0] & 0x80:
                break
        buf = config.spi_readbytes(5)
        config.digital_write(self.cs_pin, config.HIGH)
